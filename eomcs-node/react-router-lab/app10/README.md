...
export default function File() {
  ...
  return <h2>File path: {filePath}</h2>;
}
```
...
- `useParams()` 가 반환하는 객체에서 와일드카드로 매칭된 값은 `"*"` 라는 이름의 키에 담긴다.
- 그래서 `params["*"]` 처럼 대괄호 문법으로 꺼내야 한다.
...
정리하면, 와일드카드 세그먼트(`/*`)는 경로의 나머지 부분 전체를 하나의 값으로 받고 싶을 때 사용하며, 그 값은 `useParams()` 가 반환한 객체의 `"*"` 키를 통해 읽는다.
