...
- **구현이 단순하고 명확하면 → Obvious Implementation 전략**을 연속해서 사용하며 속도를 낸다.
- **어떻게 구현할지 확신이 없으면 → 작은 단계로 나눠서 진행하는 Fake It 전략**으로 시작
...
`Dollar` 객체의 부작용을 드러내는 테스트를 작성한다. 같은 `five` 객체로 `times(2)`, `times(3)`을 연속 호출했을 때 각각 10, 15가 나와야 하는데, `Dollar`는 내부 상태를 직접 변경하므로 두 번째 호출에서 30이 되어 테스트가 실패한다.
...
```
- 같은 five 객체로 두 번 연속 곱셈을 수행한다.
- 각 곱셈 후에 five.amount가 예상한 값이 되는지 검증한다.
...
```
- times()가 새 Dollar 객체를 반환하도록 변경한다.
- 이제 같은 five 객체로 두 번 연속 곱셈을 해도 five의 상태는 변하지 않는다.