...
package effectivejava.ch04.item17.exam02;

// [주제]
// 불변 클래스를 만들 때 가변 객체를 참조하는 필드를 다루는 방법
...
final class Period {
  ...
  // 생성자에서 클라이언트가 제공한 객체 레퍼런스를 가변 참조 필드에 저장해야 할 때
  public Period(Date start, Date end) {
    // 1) 먼저 복사한다 (외부 참조 차단)
    Date s = new Date(start.getTime());
    Date e = new Date(end.getTime());

    // 2) 복사본으로 불변식 검증한다 (TOCTOU 회피)
    ...
    // - 즉 외부에서 주입된 것을 복사하는 사이에 원본이 바뀔 수 있으므로, 복사본으로 검사해야 한다.
    //   ex) start를 복사하는 사이에 end 를 바꾸는 경우
