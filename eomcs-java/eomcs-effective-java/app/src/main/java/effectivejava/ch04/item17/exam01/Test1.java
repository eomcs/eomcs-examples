...
class MyObject {
  ...
  public static void read() {
    if (obj != null) {
      int i = obj.x; // x는 변경 가능 필드이므로, obj가 완전히 초기화되기 전에 이 필드에 접근하면 잘못된 값을 읽을 수 있다.
      int j = obj.y; // y는 final 필드이므로, obj가 완전히 초기화되기 전에 이 필드에 접근하더라도 항상 올바른 값을 읽는다.
    }
  }
}
...
public class Test1 {
  public static void main(String[] args) throws Exception {
    new Thread(() -> MyObject.write()).start();
    new Thread(() -> MyObject.read()).start();
