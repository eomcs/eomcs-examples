...
package effectivejava.ch11.item78.exam08;

// [주제] 스레드 안전한 프로그래밍을 지원하는 클래스 사용하기
// - java.util.concurrent.atomic 패키지에는 원자적 연산을 제공하는 클래스들이 있다.
...
public class Test {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // [정리]
    // - volatile 한정자: 스레드간 통신
    // - synchronized: 스레드간 통신 + 원자성(배타적 실행)
    // - java.util.concurrent.atomic 패키지: 스레드간 통신 + 원자성(배타적 실행)
    //   성능도 동기화 버전 보다 우수하다.

    // [멀티스레드 환경에서 위의 문제를 피하는 근본적인 방법]
    // - 불변 객체(아이템 17)만 공유하거나 아무것도 공유하지 말라.
    //   가변 데이터는 단일 스레드에서만 사용하라.
    // - 사용하려는 프레임워크와 라이브러리를 깊이 이해하라.
    //   외부 코드가 우리가 인지못한 스레드를 수행하는 복병으로 작용할 수 있다.
