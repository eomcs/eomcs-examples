...
public class Test {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // [동기화 사용 시 성능을 떨어뜨리는 주요 요인]
    // 1) 경쟁하느라 낭비되는 시간이 성능을 떨어뜨리는 주요 원인이다.
    //    락을 얻는 데 드는 CPU 시간보다 더 많은 비용이 든다.
    //    병렬로 실행할 기회를 잃고, 모든 코어가 메모리를 일관되게 보기 위한 지연시간이 이에 해당된다.
    // 3) 가상 머신이 동기화 코드의 최적화를 제한하는 것도 성능을 떨어뜨리는 주요 원인이다.
