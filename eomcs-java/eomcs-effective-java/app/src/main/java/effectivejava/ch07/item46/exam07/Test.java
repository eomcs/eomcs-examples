...
//
package effectivejava.ch07.item46.exam07;

// [주제] 분류 수집기 사용 예 - groupingBy() 사용 예
...
public class Test {
  public static void main(String[] args) throws Exception {
    ...
    // - 파라미터: 분류 함수(classifier function). 작성자를 key로 분류
    // - 리턴 값: 분류 함수의 결과(작성자)를 key로, 해당 키에 매핑된 요소(Message 객체) 리스트를 value 로 하는 맵
    Map<String, List<Message>> messagesOfWriter =
        messages.stream().collect(groupingBy(Message::writer));

    // 결과 출력
    messagesOfWriter.forEach(
        (writer, msgs) -> {
          System.out.println(writer + ": " + msgs);
        });
  }
}
