...
- `show_sql=true`이면 SELECT가 단 한 번만 출력됨을 콘솔에서 직접 확인할 수 있다.
- `c1 == c2`가 `true`인 이유는 두 번째 `find()`가 DB를 조회하지 않고 1차 캐시에서 같은 객체를 반환하기 때문이다. JDBC 방식은 `find()`마다 `new Customer()`를 생성하므로 `==`가 `false`다.
- 1차 캐시는 EntityManager 인스턴스 단위다. EM이 닫히면 캐시도 소멸하므로, 새로운 EM을 생성하면 같은 id라도 DB SELECT가 다시 실행된다.
- 실행 명령:
  ```
...
- `find()` 시점에 엔티티의 스냅샷(복사본)이 저장된다. `commit()` 직전 현재 상태와 스냅샷을 비교해 변경된 필드만 UPDATE 대상으로 선정한다.
- 변경이 없으면 스냅샷과 현재 상태가 동일하므로 UPDATE SQL이 실행되지 않는다. Hibernate가 불필요한 쿼리를 생략하는 최적화다.
- `rollback()` 시 변경 내용은 DB에 반영되지 않는다. 단, 이미 메모리상의 엔티티 객체 필드는 변경된 상태이므로 트랜잭션 롤백 후에는 해당 엔티티를 폐기하거나 다시 조회해야 한다.
- 실행 명령:
  ```
...
- `persist()` 3번 호출 후 `commit()`을 실행하면 SQL이 한꺼번에 전송된다. 이것이 **쓰기 지연(Write-Behind)**이다. 단, `IDENTITY` 전략에서는 id 채번이 즉시 필요하기 때문에 `persist()` 시점에 바로 INSERT가 실행된다.
- `em.flush()`는 SQL을 DB에 전송하지만 트랜잭션을 종료하지 않는다. `commit()` 없이 `flush()`만 하면 롤백이 가능하다.
- `FlushModeType.AUTO`(기본값)에서는 JPQL 실행 직전에도 자동으로 `flush()`가 발생한다. 따라서 `setCity("광주")` 후 JPQL을 실행하면 flush가 먼저 일어나 변경된 값이 반영된 상태에서 SELECT된다.
- 실행 명령:
  ```
