...
public class App3 {
  ...
  public static void main(String[] args) {
    ...
    try (EntityManagerFactory emf = Persistence.createEntityManagerFactory("exam20", props)) {
      ...
      try (EntityManager em = emf.createEntityManager()) {
        ...
        rows.forEach(
            row -> {
              ...
              System.out.printf("  %s[%s] %s%n", "  ".repeat(lvl - 1), row[0], row[1]);
      ...
      try (EntityManager em = emf.createEntityManager()) {
        ...
        rows.forEach(
            row -> {
              int lvl = ((Number) row[2]).intValue();
              System.out.printf(
                  "  %s[lv%d] %s (id=%s)%n", "  ".repeat(lvl - 1), lvl, row[1], row[0]);
            });
      }
